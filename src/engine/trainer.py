...
class Trainer:
    ...
    def __init__(
        ...
        self.criterion = torch.nn.CrossEntropyLoss()  # Assuming a classification task
    ...
    def save_prompt(self, epoch):
        ...
        if not self.cfg.MODEL.PROMPT.SAVE_FOR_EACH_EPOCH:
            return 
    ...
    def load_prompt(self, model, prompt_path, train_loader):
        """
        Load prompt embeddings from a saved file and apply them to the model.
        """
        self.cls_weights = train_loader.dataset.get_class_weights(
            self.cfg.DATA.CLASS_WEIGHTS_TYPE
        )
        prompt_data = torch.load(prompt_path)
        prompt_embeddings = prompt_data["shallow_prompt"]
        # Assuming the model has an attribute to directly set the embeddings
        if hasattr(model.enc.transformer, "prompt_embeddings"):
            prompt_embeddings_tensor = (
                torch.tensor(prompt_embeddings).to(self.device).detach()
            )
            model.enc.transformer.prompt_embeddings = torch.nn.Parameter(
                prompt_embeddings_tensor, requires_grad=False
            )
        else:
            raise AttributeError("Model does not support prompt embeddings directly.")

    def eval_prompt(self, data_loader, prefix, save=True):
        """
        Evaluate the model with prompt embeddings, compute metrics, log the results, and optionally save the results.
        """
        if not data_loader:
            logger.info("No data loader provided for evaluation.")
            return
        ...
        for batch_idx, data in enumerate(data_loader):
            if isinstance(data, (list, tuple)):
                ...
                logger.error("Data loader is returning data in an unrecognized format")
                return
            ...
            if (batch_idx + 1) % self.cfg.SOLVER.LOG_EVERY_N == 0:
        ...
        # Use the evaluator to compute the metrics
        self.evaluator.classify(all_outputs, all_targets, prefix, multilabel=False)

        # Optionally save results
        if save:
            results_path = os.path.join(self.cfg.OUTPUT_DIR, 'eval_results', f"{prefix}_evaluation_results.txt")
            os.makedirs(os.path.dirname(results_path), exist_ok=True)
            with open(results_path, 'w') as f:
                for key, value in self.evaluator.results.items():
                    f.write(f"{key}: {value}\n")
